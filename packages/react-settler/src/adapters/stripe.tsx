...
export function StripeApp({
  ...
  if (!hasAccess) {
    return (
      <UpgradePrompt feature={FEATURE_FLAGS.STRIPE_INTEGRATION} featureName="Stripe Integration" />
    );
  }
    ...
    >
      <ReconciliationDashboard>
        <div style={{ marginBottom: "2rem" }}>
          ...
          >
            <h1 style={{ fontSize: "1.75rem", fontWeight: 600 }}>Reconciliation Dashboard</h1>
            {transactions.length > 0 && (
              <ExportButton data={transactions} format="csv" onExport={onExport} />
            )}
          </div>
          <p style={{ color: "#6b7280", fontSize: "0.875rem" }}>Stripe Account: {accountId}</p>
        </div>
        ...
        >
          ...
          />
          <MetricCard title="Transactions" value={transactions.length} subtitle="Processed" />
          <MetricCard
            title="Match Rate"
