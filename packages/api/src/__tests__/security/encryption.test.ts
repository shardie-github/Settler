...
describe("Encryption", () => {
  ...
  describe("encrypt", () => {
    ...
    it("should use SecretsManager when available", () => {
      mockSecretsManager.getSecret.mockReturnValue(
        "hex-encoded-key-64-chars-long-for-testing-purposes-12345678901234567890"
      );
  ...
  describe("decrypt", () => {
    ...
    it("should handle legacy base64 format", () => {
      ...
      const legacyFormat = Buffer.concat([salt, iv, authTag, encrypted]).toString("base64");
  ...
  describe("round-trip compatibility", () => {
    it("should maintain compatibility across different key formats", () => {
      const testCases = [
        {
          key: "hex-encoded-key-64-chars-long-for-testing-purposes-12345678901234567890",
          format: "hex",
        },
