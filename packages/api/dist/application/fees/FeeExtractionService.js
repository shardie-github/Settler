...
class FeeExtractionService {
    ...
    extractGenericFees(transaction, tenantId) {
        ...
        for (const field of feeFields) {
            if (payload[field]) {
                const feeValue = typeof payload[field] === "number" ? payload[field] : parseFloat(payload[field] || "0");
                if (feeValue > 0) {
                    fees.push({
                        id: this.generateId(),
                        tenantId,
    ...
    calculateTotals(fees, transaction) {
        ...
        const effectiveRate = transaction.amount.value > 0 ? (totalFeesValue / transaction.amount.value) * 100 : 0;
        return {
            fees,
            totalFees,
            effectiveRate,
