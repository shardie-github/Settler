...
describe("SettlerClient", () => {
  ...
  describe("error handling", () => {
    it("should throw ValidationError for 400", async () => {
      await expect(client.request("GET", "/api/v1/error/400")).rejects.toThrow(ValidationError);
    });

    it("should throw AuthError for 401", async () => {
      await expect(client.request("GET", "/api/v1/error/401")).rejects.toThrow(AuthError);
    });
    ...
    it("should throw ServerError for 500", async () => {
      await expect(client.request("GET", "/api/v1/error/500")).rejects.toThrow();
    });
  });
});
