...
export function OnboardingExample() {
  ...
  return (
    <div>
      {variant === "variant_a" && (
        <OnboardingV2A onStepComplete={handleStepComplete} onComplete={handleComplete} />
      )}
      {variant === "variant_b" && (
        <OnboardingV2B onStepComplete={handleStepComplete} onComplete={handleComplete} />
      )}
      {variant === "control" && (
        <OnboardingControl onStepComplete={handleStepComplete} onComplete={handleComplete} />
      )}
    </div>
  );
}

function OnboardingV2A(_props: {
...
function OnboardingControl(_props: {
  onStepComplete: (step: number, name: string) => void;
  onComplete: () => void;
}) {
