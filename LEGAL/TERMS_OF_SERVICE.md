# Settler Terms of Service

**Last Updated:** January 1, 2024

## 1. Agreement to Terms

By accessing or using Settler's services ("Services"), you agree to be bound by these Terms of Service ("Terms"). If you disagree with any part of these Terms, you may not access the Services.

## 2. Definitions

- **"Settler"** refers to Settler, Inc. and its services
- **"Services"** includes:
  - Settler API (proprietary SaaS)
  - React.Settler (open-source protocol and commercial features)
  - Documentation, support, and related services
- **"You"** or **"Customer"** refers to the individual or entity using the Services
- **"OSS"** refers to open-source software licensed under MIT
- **"Commercial"** refers to paid commercial features requiring a subscription

## 3. License Tiers

### 3.1 Open Source (OSS) Tier

React.Settler OSS components are licensed under the **MIT License** and are free to use for any purpose, including commercial use, subject to the MIT License terms.

**OSS Features Include:**

- Core protocol types (`@settler/protocol`)
- Basic React components
- Config compiler
- Basic validation and security
- Mobile and accessibility support

### 3.2 Commercial Tier

Commercial features require a paid subscription ($99/month or $990/year).

**Commercial Features Include:**

- Platform integrations (Shopify, Stripe, MCP)
- Virtualized tables
- Advanced telemetry
- Audit logging
- Webhook manager

**Commercial License Terms:**

- Subscription required for Commercial features
- License is non-transferable
- Subscription renews automatically unless cancelled
- Cancellation effective at end of billing period

### 3.3 Enterprise Tier

Enterprise features require a custom contract with Settler.

**Enterprise Features Include:**

- SSO and RBAC
- Custom integrations
- White-label options
- Dedicated support
- SLA guarantees

**Enterprise License Terms:**

- Governed by separate Enterprise Agreement
- Custom pricing and terms
- Annual or multi-year contracts

## 4. Settler API SaaS

### 4.1 Usage Rights

Subject to payment of applicable fees, Settler grants you a limited, non-exclusive, non-transferable right to access and use the Settler API SaaS during the subscription term.

### 4.2 Usage Limits

Usage limits are defined by your subscription tier:

- **Starter:** Up to 1,000 reconciliation jobs/month
- **Pro:** Up to 10,000 reconciliation jobs/month
- **Enterprise:** Unlimited (subject to fair use)

### 4.3 Overages

Usage exceeding your tier limits may result in:

- Automatic upgrade to next tier, or
- Additional charges at overage rates, or
- Service throttling

## 5. Payment Terms

### 5.1 Subscription Fees

- Commercial subscriptions: $99/month or $990/year (billed annually)
- Settler API SaaS: Usage-based + subscription fees
- Enterprise: Custom pricing per contract

### 5.2 Payment

- Payment due in advance for subscription periods
- Accepted payment methods: Credit card, ACH, wire transfer (Enterprise)
- Failed payments may result in service suspension

### 5.3 Refunds

- No refunds for partial subscription periods
- Refunds may be provided at Settler's discretion for service failures
- Enterprise contracts subject to separate refund terms

### 5.4 Price Changes

- Settler may change prices with 30 days' notice
- Existing subscriptions honored until renewal
- Price increases apply to renewals

## 6. Acceptable Use

### 6.1 Prohibited Uses

You may not:

- Reverse engineer, decompile, or disassemble proprietary components
- Use Services to violate any laws or regulations
- Transmit malicious code or viruses
- Attempt to gain unauthorized access to Services
- Use Services to compete with Settler
- Resell Services without authorization

### 6.2 OSS Usage

OSS components may be used freely subject to MIT License terms. Commercial features require a paid subscription.

## 7. Intellectual Property

### 7.1 Settler's IP

Settler retains all rights, title, and interest in:

- Settler API (proprietary)
- Commercial features of React.Settler
- Documentation and materials
- Trademarks and branding

### 7.2 Your IP

You retain all rights to:

- Your data and content
- Applications built using Settler
- Custom integrations and modifications (OSS components)

### 7.3 OSS License

OSS components are licensed under MIT License. You may:

- Use, modify, and distribute OSS components
- Use OSS components in commercial products
- Create derivative works

## 8. Data and Privacy

### 8.1 Data Ownership

You own all data you submit to Settler. Settler does not claim ownership of your data.

### 8.2 Data Processing

Settler processes your data only to provide Services. See [Privacy Policy](./PRIVACY_POLICY.md) for details.

### 8.3 Data Security

Settler implements industry-standard security measures but cannot guarantee absolute security.

## 9. Service Availability

### 9.1 SLA

- **Starter:** 99.5% uptime
- **Pro:** 99.9% uptime
- **Enterprise:** 99.99% uptime (per contract)

### 9.2 Service Modifications

Settler may modify, suspend, or discontinue Services with reasonable notice, except for Enterprise customers (per contract terms).

## 10. Support

### 10.1 Support Levels

- **OSS:** Community support (GitHub, Discord)
- **Commercial:** Email support (48-hour response)
- **Enterprise:** Dedicated support (24/7, per contract)

### 10.2 Support Scope

Support includes:

- Technical assistance with Services
- Bug fixes and updates
- Documentation and guides

Support does not include:

- Custom development
- Integration consulting (available as Professional Services)
- Training (available as Professional Services)

## 11. Termination

### 11.1 By You

You may cancel your subscription at any time. Cancellation effective at end of billing period. No refunds for partial periods.

### 11.2 By Settler

Settler may suspend or terminate Services if:

- You breach these Terms
- Payment is overdue
- You engage in prohibited uses
- Required by law

### 11.3 Effect of Termination

Upon termination:

- Access to Commercial features ceases
- OSS components remain available under MIT License
- Data export available for 30 days
- All fees owed remain due

## 12. Warranties and Disclaimers

### 12.1 Warranties

Settler warrants that:

- Services will perform substantially as described
- Settler has right to grant licenses

### 12.2 Disclaimers

**EXCEPT AS EXPRESSLY PROVIDED, SERVICES ARE PROVIDED "AS IS" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED, INCLUDING WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, OR NON-INFRINGEMENT.**

OSS components provided without warranty per MIT License.

## 13. Limitation of Liability

**TO THE MAXIMUM EXTENT PERMITTED BY LAW:**

- Settler's total liability shall not exceed fees paid in the 12 months preceding the claim
- Settler is not liable for indirect, incidental, or consequential damages
- Settler is not liable for data loss or corruption

## 14. Indemnification

You agree to indemnify Settler against claims arising from:

- Your use of Services
- Your breach of these Terms
- Your violation of any laws or third-party rights

## 15. Governing Law

These Terms are governed by the laws of [Jurisdiction], without regard to conflict of law principles.

## 16. Changes to Terms

Settler may modify these Terms with 30 days' notice. Continued use after changes constitutes acceptance.

## 17. Contact

For questions about these Terms:

- **Email:** legal@settler.dev
- **Address:** [Company Address]

## 18. Entire Agreement

These Terms constitute the entire agreement between you and Settler regarding Services, superseding all prior agreements.

---

**By using Settler's Services, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service.**
