# Seed Round Due Diligence Index

**Version:** 1.0  
**Date:** January 2026  
**Company:** Settler, Inc.  
**Round:** Seed Round ($2M)  
**Status:** Preparation Phase

---

## Executive Summary

This document serves as a comprehensive index of all materials required for Seed Round due diligence. All documents should be organized in a Virtual Data Room (VDR) with appropriate access controls and audit logging.

**VDR Platform:** [To be determined - Dropbox, Google Drive, or dedicated VDR platform]  
**VDR Access:** [To be provided to investors upon term sheet execution]  
**Last Updated:** [Date of last document upload]

---

## Section 1: Corporate & Legal Documents

### 1.1 Corporate Formation & Structure

**Required Documents:**

- [ ] Certificate of Incorporation (Delaware C-Corp)
- [ ] Certificate of Good Standing (current, within 30 days)
- [ ] Bylaws (current version)
- [ ] Operating Agreement (if applicable)
- [ ] State registrations (qualification to do business in other states)
- [ ] Corporate minute book (all board and shareholder meeting minutes)
- [ ] Stock ledger and cap table (Carta or equivalent)
- [ ] Stock option plan documents (if applicable)

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** Legal Counsel  
**Last Updated:** [Date]

---

### 1.2 Capitalization Table

**Required Documents:**

- [ ] Current cap table (fully diluted, showing all equity holders)
- [ ] Cap table history (all previous rounds, if any)
- [ ] Stock option pool details (authorized, issued, reserved, available)
- [ ] Outstanding warrants or convertible securities
- [ ] SAFE/convertible note agreements (if applicable)
- [ ] Pro forma cap table (post-Seed Round)

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CFO/Founder  
**Last Updated:** [Date]

**Key Metrics:**

- Total shares outstanding: [Number]
- Option pool size: [Number] shares ([X]%)
- Fully diluted ownership: [Breakdown by holder]

---

### 1.3 Intellectual Property (IP)

**Required Documents:**

- [ ] IP Assignment Agreements (all founders and employees)
- [ ] Patent applications (if any)
- [ ] Trademark registrations and applications
- [ ] Copyright registrations
- [ ] Open source license compliance audit
- [ ] Third-party IP licenses (software, APIs, etc.)
- [ ] IP ownership verification (code, designs, documentation)
- [ ] Non-disclosure agreements (NDAs) with contractors/consultants

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** Legal Counsel + CTO  
**Last Updated:** [Date]

**IP Inventory:**

- Code repositories: [List with ownership verification]
- Design assets: [List with ownership verification]
- Documentation: [List with ownership verification]
- Third-party dependencies: [License audit report]

---

### 1.4 Employment & Contractor Agreements

**Required Documents:**

- [ ] Employment agreements (all employees, including founders)
- [ ] Contractor/consultant agreements
- [ ] Non-compete agreements (if applicable)
- [ ] Confidentiality agreements (all employees and contractors)
- [ ] Offer letters (all employees)
- [ ] Employee handbook
- [ ] Equity grant agreements (stock options, RSUs, etc.)
- [ ] Vesting schedules (all equity holders)

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** Legal Counsel + HR  
**Last Updated:** [Date]

**Key Terms:**

- Founder vesting: 4-year vest, 1-year cliff (standard)
- Employee vesting: [Standard terms]
- Non-compete: [Yes/No, applicable states]

---

### 1.5 Advisory Board & Key Relationships

**Required Documents:**

- [ ] Advisory board agreements (all advisors)
- [ ] Advisor equity grants (if applicable)
- [ ] Strategic partnership agreements
- [ ] Key customer contracts (if material)
- [ ] Vendor agreements (critical infrastructure providers)

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CEO  
**Last Updated:** [Date]

**Advisory Board:**

- [Advisor 1]: [Name, Title, Company] - [Domain expertise]
- [Advisor 2]: [Name, Title, Company] - [Domain expertise]
- [Advisor 3]: [Name, Title, Company] - [Domain expertise]

---

### 1.6 Legal Compliance & Regulatory

**Required Documents:**

- [ ] Business licenses and permits
- [ ] Tax identification numbers (EIN, state tax IDs)
- [ ] Compliance certifications (SOC 2, GDPR, PCI-DSS status)
- [ ] Privacy policy and terms of service
- [ ] Data processing agreements (with customers, if applicable)
- [ ] Regulatory filings (if applicable to industry)
- [ ] Litigation history (if any)
- [ ] Insurance policies (general liability, D&O, cyber)

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** Legal Counsel  
**Last Updated:** [Date]

**Compliance Status:**

- SOC 2 Type II: [ ] In Progress (target: Month 9) | [ ] Not Started
- GDPR: [ ] Compliant | [ ] In Progress
- PCI-DSS: [ ] Compliant | [ ] In Progress
- Other: [List]

---

## Section 2: Financial Documents

### 2.1 Historical Financials

**Required Documents:**

- [ ] Financial statements (P&L, Balance Sheet, Cash Flow) - all periods since inception
- [ ] Bank statements (last 12 months)
- [ ] Credit card statements (business accounts, last 12 months)
- [ ] Tax returns (all years, if applicable)
- [ ] Accounting records (chart of accounts, general ledger)
- [ ] Auditor's reports (if any)

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CFO/Accountant  
**Last Updated:** [Date]

**Financial Summary:**

- Total revenue (YTD): $[Amount]
- Total expenses (YTD): $[Amount]
- Current cash balance: $[Amount]
- Monthly burn rate: $[Amount]
- Runway (months): [Number]

---

### 2.2 Financial Projections

**Required Documents:**

- [ ] 3-year financial model (bottom-up, with assumptions)
- [ ] Monthly cash flow projections (next 24 months)
- [ ] Scenario analysis (Base Case, Best Case, Worst Case)
- [ ] Unit economics model (CAC, LTV, Payback Period, Gross Margin)
- [ ] Use of Funds breakdown (by category and timeline)
- [ ] Break-even analysis

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CFO/Founder  
**Last Updated:** [Date]

**Key Assumptions:**

- Average customer value: $[Amount]/month
- Customer acquisition cost (CAC): $[Amount]
- Lifetime value (LTV): $[Amount]
- LTV:CAC ratio: [X]:1
- Gross margin: [X]%
- Monthly churn: [X]%

---

### 2.3 Accounting & Tax

**Required Documents:**

- [ ] Chart of accounts
- [ ] Accounting policies and procedures
- [ ] Tax returns (all years)
- [ ] Tax election documents (S-Corp, C-Corp, etc.)
- [ ] Sales tax registrations (if applicable)
- [ ] Payroll tax records
- [ ] 1099 forms (contractors, if applicable)

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** Accountant  
**Last Updated:** [Date]

---

## Section 3: Technical & Product Documents

### 3.1 Technical Architecture

**Required Documents:**

- [ ] System architecture diagram
- [ ] Infrastructure documentation (hosting, CDN, databases)
- [ ] Security architecture and threat model
- [ ] API documentation (OpenAPI/Swagger spec)
- [ ] Database schema documentation
- [ ] Technology stack overview
- [ ] Scalability and performance testing reports

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CTO  
**Last Updated:** [Date]

**Technical Stack:**

- Backend: [List]
- Frontend: [List]
- Database: [List]
- Infrastructure: [List]
- Third-party services: [List]

---

### 3.2 Code Quality & Security

**Required Documents:**

- [ ] Code quality report (SonarQube, CodeClimate, etc.)
- [ ] Security audit report (OWASP Top 10 compliance)
- [ ] Penetration testing report (if completed)
- [ ] Dependency audit (vulnerability scan)
- [ ] CI/CD pipeline documentation
- [ ] Testing coverage report (unit, integration, E2E)
- [ ] Incident response plan

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CTO  
**Last Updated:** [Date]

**Quality Metrics:**

- Code coverage: [X]%
- Security vulnerabilities: [Number] (High: [X], Medium: [X], Low: [X])
- Technical debt: [Assessment]

---

### 3.3 Product Documentation

**Required Documents:**

- [ ] Product roadmap (18-24 months)
- [ ] Feature specifications (core features)
- [ ] User documentation and help center
- [ ] API documentation (developer docs)
- [ ] Product demo video
- [ ] User onboarding flow documentation
- [ ] Product analytics dashboard (screenshots/summary)

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** Product Manager/Founder  
**Last Updated:** [Date]

---

### 3.4 Data & Privacy

**Required Documents:**

- [ ] Data privacy policy
- [ ] Data processing agreements (templates)
- [ ] GDPR compliance documentation
- [ ] Data retention policies
- [ ] Security incident response plan
- [ ] Backup and disaster recovery plan

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CTO + Legal  
**Last Updated:** [Date]

---

## Section 4: Business & Market Documents

### 4.1 Market Analysis

**Required Documents:**

- [ ] Market sizing analysis (TAM/SAM/SOM)
- [ ] Competitive analysis (detailed competitive matrix)
- [ ] Industry research reports (third-party sources)
- [ ] Market trends and growth projections
- [ ] Customer segmentation analysis

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CEO/Founder  
**Last Updated:** [Date]

**Market Metrics:**

- TAM: $[Amount] (Total Addressable Market)
- SAM: $[Amount] (Serviceable Addressable Market)
- SOM: $[Amount] (Serviceable Obtainable Market)
- Market growth rate: [X]% YoY

---

### 4.2 Go-to-Market Strategy

**Required Documents:**

- [ ] GTM plan (18-24 months)
- [ ] Sales process documentation
- [ ] Marketing strategy and channels
- [ ] Partner program documentation
- [ ] Pricing strategy and model
- [ ] Customer acquisition strategy

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CEO/Founder  
**Last Updated:** [Date]

---

### 4.3 Traction & Customer Validation

**Required Documents:**

- [ ] Customer list (with revenue, if applicable)
- [ ] Customer testimonials and case studies
- [ ] Letters of Intent (LOIs) from potential customers
- [ ] Customer interview synthesis (10+ interviews)
- [ ] Product-Market Fit (PMF) validation report
- [ ] Usage metrics and analytics (anonymized)
- [ ] Net Promoter Score (NPS) and customer satisfaction data

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CEO/Founder  
**Last Updated:** [Date]

**Traction Metrics:**

- Total users: [Number]
- Paying customers: [Number]
- MRR: $[Amount]
- ARR: $[Amount]
- LOIs secured: [Number]
- Customer interviews: [Number]

---

### 4.4 Team & Hiring

**Required Documents:**

- [ ] Team bios and resumes (all employees)
- [ ] Organizational chart
- [ ] 18-Month Post-Funding Hiring Roadmap (see separate document)
- [ ] Job descriptions (for planned hires)
- [ ] Compensation structure and philosophy
- [ ] Employee equity grant summary

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CEO/Founder  
**Last Updated:** [Date]

**Current Team:**

- [Founder 1]: [Name, Title] - [Background]
- [Founder 2]: [Name, Title] - [Background]
- [Employee 1]: [Name, Title] - [Background]
- [Advisors]: [List]

---

## Section 5: Investor Materials

### 5.1 Pitch Materials

**Required Documents:**

- [ ] Seed Round Pitch Deck (see separate document)
- [ ] Executive summary (1-2 page)
- [ ] Product demo video
- [ ] Financial model summary (key slides)
- [ ] Competitive analysis summary

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CEO/Founder  
**Last Updated:** [Date]

---

### 5.2 Term Sheet & Legal

**Required Documents:**

- [ ] Term sheet (when received)
- [ ] Legal opinion (from company counsel)
- [ ] Investor rights agreement (template)
- [ ] Shareholders agreement (template)
- [ ] Board composition proposal

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** Legal Counsel  
**Last Updated:** [Date]

---

## Section 6: Operational Documents

### 6.1 Operations & Processes

**Required Documents:**

- [ ] Standard Operating Procedures (SOPs) - 5 core procedures
  - [ ] Sales SOP
  - [ ] Customer Success SOP
  - [ ] Bug Fix SOP
  - [ ] Onboarding SOP
  - [ ] Support SOP
- [ ] CRM/Support system documentation
- [ ] Customer health scoring methodology
- [ ] Incident response runbook

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** Operations Lead  
**Last Updated:** [Date]

---

### 6.2 Metrics & Reporting

**Required Documents:**

- [ ] Key Performance Indicators (KPIs) dashboard
- [ ] Unit economics dashboard (LTV:CAC, NRR, Churn)
- [ ] Monthly investor update template
- [ ] Metrics framework documentation

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CFO/Founder  
**Last Updated:** [Date]

**Key Metrics:**

- MRR: $[Amount]
- ARR: $[Amount]
- LTV:CAC: [X]:1
- NRR: [X]%
- Monthly churn: [X]%
- Gross margin: [X]%

---

## Section 7: Risk Management

### 7.1 Risk Assessment

**Required Documents:**

- [ ] Risk register (comprehensive risk matrix)
- [ ] Risk mitigation plans
- [ ] Business continuity plan
- [ ] Disaster recovery plan
- [ ] Insurance coverage summary

**Status:** [ ] Complete | [ ] In Progress | [ ] Not Started  
**Owner:** CEO/Founder  
**Last Updated:** [Date]

**Key Risks Identified:**

- [Risk 1]: [Description] - [Mitigation]
- [Risk 2]: [Description] - [Mitigation]
- [Risk 3]: [Description] - [Mitigation]

---

## Document Checklist Summary

### Overall Status

- **Corporate & Legal:** [ ] Complete | [ ] In Progress | [ ] Not Started
- **Financial:** [ ] Complete | [ ] In Progress | [ ] Not Started
- **Technical & Product:** [ ] Complete | [ ] In Progress | [ ] Not Started
- **Business & Market:** [ ] Complete | [ ] In Progress | [ ] Not Started
- **Investor Materials:** [ ] Complete | [ ] In Progress | [ ] Not Started
- **Operational:** [ ] Complete | [ ] In Progress | [ ] Not Started
- **Risk Management:** [ ] Complete | [ ] In Progress | [ ] Not Started

### Critical Path Items (Must Complete Before Fundraising)

1. [ ] Corporate formation (Delaware C-Corp)
2. [ ] Cap table finalized
3. [ ] IP assignments complete (all founders/employees)
4. [ ] Financial model complete (3-year projections)
5. [ ] 5+ LOIs or paid pilots secured
6. [ ] Pitch deck finalized
7. [ ] VDR set up with all documents organized

---

## VDR Access Instructions

**For Investors:**

1. Request VDR access via [email/contact]
2. Sign NDA (if not already executed)
3. Receive VDR link and credentials
4. Access expires [X] days after term sheet expiration (if not executed)

**For Company:**

- Maintain audit log of all VDR access
- Update documents as they become available
- Notify investors of significant updates
- Ensure all documents are current (within 30 days for financials, 90 days for other docs)

---

## Red Flag Mitigation Checklist

### Financial Red Flags

- [ ] Transparent about burn rate and runway
- [ ] Clear path to positive unit economics documented
- [ ] All financial assumptions validated and sourced
- [ ] No undisclosed liabilities or obligations

### Technical Red Flags

- [ ] Security audit complete (or in progress with timeline)
- [ ] Tech debt management plan documented
- [ ] Clear roadmap for future security certifications (SOC 2)
- [ ] Code quality metrics meet industry standards

### Legal Red Flags

- [ ] Clean title for all IP (no disputes)
- [ ] No founder non-competes or legal conflicts
- [ ] All employment agreements in place
- [ ] No pending litigation or regulatory issues

### Market Red Flags

- [ ] Valuation based on validated traction, not hype
- [ ] Clear market opportunity (TAM >$1B)
- [ ] Defensible competitive position
- [ ] Realistic growth projections

---

## Document Update Schedule

- **Weekly:** Financials (if material changes), Traction metrics
- **Monthly:** Team updates, Product roadmap updates
- **Quarterly:** Market analysis, Competitive analysis
- **As Needed:** Legal documents, Technical documentation, Operational procedures

---

**Document Owner:** CEO/Founder  
**Last Updated:** [Date]  
**Next Review:** [Date - should be weekly during fundraising]

---

_This index should be updated regularly as documents are completed and added to the VDR. All team members should be aware of their responsibilities for document preparation and updates._
