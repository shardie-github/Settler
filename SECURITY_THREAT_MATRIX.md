...
const refreshToken = jwt.sign({ userId, type: "refresh" }, refreshSecret, { expiresIn: "7d" });
```
...
async function verifyWebhookSignature(
  ...
  switch (adapter) {
    ...
    case "shopify":
      ...
      const hmac = crypto.createHmac("sha256", secret).update(payloadBuffer).digest("base64");
...
router.post("/receive/:adapter", async (req, res) => {
  ...
  const isValid = await verifyWebhookSignature(adapter, rawBody, signature, webhookConfig.secret);
...
async function fetchWithRetry(
  ...
  try {
    return await pRetry(() => adapter.fetch({ ...options, signal: controller.signal }), {
      retries: maxRetries,
      minTimeout: 1000,
      maxTimeout: 10000,
      factor: 2,
...
const stripeBreaker = new CircuitBreaker(() => stripeAdapter.fetch(options), breakerOptions);
...
_This document is confidential and intended for internal security assessment only._
